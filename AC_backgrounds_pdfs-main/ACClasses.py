...
def GaussianFilterGPT(R, S1, S2, binwidth, maxS1Range, maxS2Range, ENF):
    S1c, S2c = np.meshgrid(np.arange(1, maxS1Range + 1), np.arange(1, maxS2Range + 1), indexing='ij') # Use 'ij' indexing
...
'''
def fS1CoincidenceWindow(height):
	return -50. + 1.*height
...
class Detector:
	def __init__(self,
		...
		self.TPCArea = TPCHeight * 2*np.pi*TPCRadius + 2*np.pi*TPCRadius*TPCRadius
...
class ParticleBackground:
	def __init__(self,
		...
		self.A_neutrons_fiducial = A_neutrons_fiducial
...
class InstrumentalBackground:
	...
	def DNCoincidenceRate(self, multiplicity, includeUnxeplainedLonePhotons=True):
		...
		return AllCountsToConsider * PoissonV(ExpectedEventsInWindow,multiplicity-1) * SecondsPerYear #Events per year
